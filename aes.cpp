...
const uint8_t Aes128::mult_two_table[256] = {
...
const uint8_t Aes128::mult_three_table[256] = {
...
void rot_word_left(uint8_t * word) {
	uint8_t temp_var;
	temp_var = word[0];
	word[0] = word[1];
	word[1] = word[2];
...
uint8_t Aes128::mult_two(uint8_t val) {
	return mult_two_table[val];
}

uint8_t Aes128::mult_three(uint8_t val) {
	return mult_three_table[val];
}
...
void Aes128::mix_column() {
	...
	for(int j = 0; j < 4; ++j) {
		temp_array[0] = mult_two(states[0][j]) ^ mult_three(states[1][j]) ^ states[2][j] ^ states[3][j];
		temp_array[1] = states[0][j] ^ mult_two(states[1][j]) ^ mult_three(states[2][j]) ^ states[3][j];
		temp_array[2] = states[0][j] ^ states[1][j] ^ mult_two(states[2][j]) ^ mult_three(states[3][j]);
		temp_array[3] = mult_three(states[0][j]) ^ states[1][j] ^ states[2][j] ^ mult_two(states[3][j]);
